...
cocos2d::Vec2 Square::calcPosInGroup(Vec2 squareSize)
{
...
{
	Vec2 _leftBottom = calcPosInGroup(squareSize);
	Vec2 _squareLeftBottom = _leftBottom + Vec2(2, 2);

	Vec2 _rightTop = _leftBottom + squareSize - Vec2(1, 1);
	Vec2 _squareRightTop = _rightTop - Vec2(2, 2);
...
void Square::drawFrame(cocos2d::DrawNode* drawNode, Vec2 squareSize, Color4F frameColor)
{
	...
	Vec2 _rightTop = Vec2(squareSize.x * (this->getIndexX() + 1) -1 , squareSize.y * (this->getIndexY() + 1) -1 );
